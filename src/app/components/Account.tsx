    ...
    <div style={{display: "flex column"}}>
       ...
       {Boolean(license_key) && <div style={{display: "flex", flexDirection: "row", width: "100%", padding: "15px"}}>
        ...
        {Boolean(!license_key) && <div style={{display: "flex", flexDirection: "column", width: "100%", padding: "15px"}}>
            ...
            <div style={{width: "100%"}} ><input onInput={(e)=>setLicense(e.currentTarget.value)} disabled={processing} className="text-input border-bottom" type="text"/></div>
            {Boolean(error) && <div style={{width: "100%", color: "#ff0d31"}}><p>{error}</p></div>}
            <div style={{marginTop: "10px"}}><Button onClick={handleSubmit} disabled={processing} secondary style={{ fontSize: "10px", height: "20px", lineHeight: "10px" }}>Submit</Button></div>
        </div>}
