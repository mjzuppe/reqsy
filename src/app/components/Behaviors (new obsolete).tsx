...
const ChooseBehaviorView = () => {
    ...
    <div style={{ display: "flex", flexDirection: "row",  height: "100%", width: "100%"}} >
        <div style={{padding: "5px"}}><Button onClick={()=>{}} >Get suggested</Button></div>
        <div style={{padding: "5px"}}><Button onClick={()=>{}}>Manually create</Button></div>
    </div>
</div>
)
}
...
const SelectBehavior = () => 
    ...
    <Select style={{width: "100px"}} id="behavior-element-type-select" defaultValue={"null"} options={[{label: "element type", value: null},{label: "select", value: "select"}]}/>
</div>
...
export const Behaviors = (props: {selectionData: any, currentViewData: any}) => {
    ...
    const isNew = true;
    const view = isNew? (selectBehaviorView? <SelectBehavior/> : <div onClick={()=>setSelectBehaviorView(true)}><ChooseBehaviorView /></div>) : <MutateBehaviorView />;
    return view;
}