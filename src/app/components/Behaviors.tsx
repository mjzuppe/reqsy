...
const elementCategory = commonBehaviors
  .map((e: any) => ({ label: `<${e.element}/>`, value: e.element }))
...
let allParametersOptions = Array.from(allParametersSet)
  .map((e: any) => ({ value: e }))
...
const ToolTip = (props: { type: string; description: string }) => (
  <div className="tool-tip">
    <div style={{ width: '100%' }}>
      ...
      <span style={{ fontWeight: 'normal' }}>{props.type}</span>
    </div>
    <div style={{ width: '100%' }}>
      <span style={{ fontWeight: 'bold' }}>Notes/Validation: </span>
      <span style={{ fontWeight: 'normal' }}>{props.description}</span>
    </div>
  </div>
);

const BehaviorRow = (props: {
  db: any;
  disabled: boolean;
  readOnly: boolean;
  index: number;
  handleUpdate: (e: any) => any;
  handleDelete?: (e: any) => any;
  behavior?: { key: string; value: string };
}) => {
  ...
  const variableInUse = Object.keys(db.variable)
    .filter((v: any) => db.variable[v].label === value)
    .map((v: any) => ({ ...db.variable[v] }));
    ...
    >
      <div style={{ width: '120px' }}>
        <TextboxAutocomplete
          ...
          onBlur={() => handleUpdate({ key: param, value, index })}
          filter
          variant="underline"
          placeholder="parameter..."
          value={param}
          onInput={(e) => setParam(e.currentTarget.value)}
          options={allParametersOptions}
        />
      </div>
      <div
        style={{
        ...
        {Boolean(variableInUse.length) && showDetail ? (
          ...
          >
            ...
            <TextboxAutocomplete
              ...
              style={{ color: Boolean(variableInUse.length) ? '#0d99ff' : 'initial' }}
              disabled={disabled || readOnly}
              onBlur={() => handleUpdate({ key: param, value, index })}
              filter
              variant="underline"
      ...
      {!readOnly && !disabled && (
        <Menu
          danger
          marginLeft={'-28px'}
          onClick={() => handleDelete({ key: behavior.key, value: behavior.value, index })}
          options={['delete?']}
          trigger={
            <IconButton>
              <IconEllipsis32 />
...
const BehaviorRows = (props: {
  db: any;
  disabled: boolean;
  readOnly: boolean;
  updateBehavior: (any) => any;
  deleteBehavior: (any) => any;
  isNew;
  behaviors;
}) => {
  ...
  return (
    <div style={{ width: '100%', paddingRight: '5px' }}>
      ...
      >
        {!behaviors.length && (disabled || readOnly) && (
          <div style={{ marginLeft: '10px' }}>No behaviors are set for this element.</div>
        )}
        {newBehaviorRow && (
          <BehaviorRow index={null} db={db} handleUpdate={updateBehavior} disabled={disabled} readOnly={readOnly} />
        )}
        {behaviors.map((b: any, i: number) => (
          <BehaviorRow
            key={`behavior-row-${i}`}
            index={i}
            handleDelete={deleteBehavior}
            db={db}
            behavior={b}
            handleUpdate={updateBehavior}
            disabled={disabled}
            readOnly={readOnly}
          />
        ))}
...
const CreateBehaviors = (props: { handleClick: (any) => any }) => {
  ...
  return (
    <div
      style={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'flex-start', paddingBottom: '10px' }}
    >
      <Button
        onClick={() => handleClick(true)}
        style={{ marginLeft: '5px', fontSize: '10px', height: '20px', lineHeight: '10px' }}
      >
        Use recommended
      </Button>
      <Button
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        onClick={() => props.handleSubmit(select)}
        style={{ marginLeft: '5px', fontSize: '10px', height: '20px', lineHeight: '10px' }}
      >
        submit
      </Button>
...
export const Behaviors = (props: {
  ...
  const handleUpdateBehavior = async (e: any) => {
    ...
    if (isNew) payload = [{ id: conditionId, value: [newRecord] }];
      ...
      payload = [
        {
          id: conditionId,
          value: [newRecord, ...selectionData.behavior.filter((n: any) => n.id === conditionId)[0].value],
        },
      ];
    else {
      ...
      payload = [
        { id: conditionId, value: currentBehaviors },
        ...selectionData.behavior.filter((n: any) => n.id !== conditionId),
      ];
    }
  ...
  const createBehaviorTemplate = async (elementType: string) => {
    ...
    const payload = [
      { id: conditionId, value: [...templateValues, ...attributes.map((a: any) => ({ key: a, value: '' }))] },
    ];
    await controller({ func: 'write', data: { model: 'selection', key: 'behavior', value: payload } });
  };

  const view =
    displayRows || disabled || readOnly ? (
      <BehaviorRows
      ...
      <SuggestBehavior handleSubmit={createBehaviorTemplate} handleCancel={() => setSelectBehaviorView(false)} />
    ) : (
