...
export const Condition = (props: {
  ...
  const view =
    !allConditions.length && (disabled || readOnly) ? (
      <div style={{ marginLeft: '5px' }}>This element has no conditions set.</div>
    ) : createConditionState || editConditionState ? (
      <TextInput
        defaultValue={textCondition}
        invalidlist={allConditions.map((c: any) => c.label)}
        id={'condition-text-input'}
