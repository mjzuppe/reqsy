...
export const General = (props: { selectionData: any; db: any; readOnly: boolean }) => {
  ...
  const options: Array<TextboxAutocompleteOption> = Object.keys(tagsObject).map((key: string) => ({
    value: key,
    label: tagsObject[key].label,
  }));
  const { label, tag } = selectionData;
  const [currentTag, setCurrentTag] = useState(tag || []);

  const invalidLabels = (current: string) =>
    Object.values(libraryObject)
      .map((libData: any) => libData.label)
      .filter((libLabel: string) => libLabel !== current);
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
        ...
        <TextInput
          disabled={readOnly}
          invalidlist={invalidLabels(label)}
