...
const InspectorItem = (props: {
  ...
  readOnly: boolean;
  expanded: boolean;
  handleExpanded: (e: any) => any;
  disabled?: boolean;
  currentViewValue?: string;
  currentView?: (e: string) => any;
}) => {
  ...
  return (
    <div className={`items-list-item ${title !== 'Notes' && 'items-border-bottom'}`}>
      ...
      <div className={`items-list-item-expanded ${expanded ? '' : 'hide'}`}>{view}</div>
    </div>
  );
};
...
const NotRegisteredView = (props: { db: any }) => {
  ...
  return linkView ? (
    <LinkToComponentView setLinkView={(e: any) => setLinkView(e)} db={props.db} />
  ) : (
    <RegisterOrLinkView setLinkView={(e: any) => setLinkView(e)} db={props.db} />
  );
};
    ...
    <div id="action-container">
      ...
      >
        ...
        <div style={{ padding: '5px' }}>
          <Button onClick={handleRegisterComponent} style={{ width: '180px' }}>
            Register as new element
          </Button>
        </div>
        {!libraryIsEmpty && (
          <div style={{ padding: '5px' }}>
            <Button onClick={handleLinkViewClick} style={{ width: '180px' }}>
              Link to library element
            </Button>
          </div>
        )}
      </div>
...
const LinkToComponentView = (props: { db: any; setLinkView: (any) => any }) => {
  ...
  const options = Object.keys(db.library)
    .filter((e: any) => db.library[e].active)
    .map((e: any) => ({ label: db.library[e].label, value: e }));
...
export const Inspector = (props: { selectionData: any; db: any; readOnly: boolean; updated: number }) => {
  ...
  return selectionData === undefined || (selectionData.init === '' && readOnly) || updated !== lastUpdated ? (
    <NoSelectionView />
  ) : selectionData.init === '' ? (
    <NotRegisteredView db={db} />
  ) : (
    <div id="action-container">
      <div style={{ display: 'flex', marginLeft: '10px', height: '40px', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignSelf: 'center' }}>
          ...
          {Boolean(conditionView && conditionView !== 'default') && (
            <div style={{ fontWeight: 'normal', fontSize: '0.8em' }}>
              {conditionView === 'default' ? '' : `${conditionView}`}
            </div>
          )}
        </div>
        {/* <IconLockLocked32/> */}
      ...
      <div className="action-container-subcontainer">
        <div className="items-list">
          ...
          <InspectorItem
            ...
            title="Behaviors"
            selectionData={sourceData}
            currentViewValue={conditionView}
            db={db}
