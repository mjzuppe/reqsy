...
export const Library = (props: { db: any; readOnly: boolean }) => {
  ...
  const libraryFiltered = search
    ? libraryKeys.filter((key: string) => library[key].label.toLowerCase().includes(search.toLowerCase()))
    : libraryKeys;
  ...
  return (
    <div id="action-container">
      <div className="action-container-content">
        <p>
          <strong>Library.</strong> Manage registered elements.
        </p>
      </div>
            ...
            return (
              <div key={`library-item-${key}`} className="items-list-item items-border-bottom">
                <div className="items-list-item-alwaysdisplay">
                  <div id={`library-item-key-${key}`} onClick={handleSelect} className="library-label">
                    {label}
                  </div>
                  {!expanded.includes(i) ? (
                    <div>
                ...
                <div className={`items-list-item-expanded ${expanded.includes(i) ? '' : 'hide'}`}>
                  <div
                    style={{ alignItems: 'center', width: '100%', display: 'flex', justifyContent: 'space-between' }}
                  >
                    <div className="flex-center" title="source component" style={{ padding: '0 10px 0 10px' }}>
                      <IconLayerComponent16 />
                      <p>{type || 'null'}</p>
                    </div>
                    <div className="flex-center" title="frame location" style={{ padding: '0 10px 0 10px' }}>
                      <IconLayerFrame16 />
                      <p>{location || 'null'}</p>
                    </div>
                    {Boolean(!readOnly) && (
                  ...
                  {Boolean(tag.length) && (
                    <div title="associated tags" style={{ width: '100%', padding: '0 10px 0 10px' }}>
