...
export const Notes = (props: {selectionData: any, currentViewValue: string}) => {
    const { selectionData, currentViewValue } = props;
    ...
    return(
        <div key={`inspector-note-${conditionId}`} style={{  width: "100%", display: "flex", flexDirection: "column", alignItems: "flex-start", paddingBottom: "10px"}}>
            <textarea onBlur={handleNoteUpdate} defaultValue={note} maxLength={256} onKeyDown={(e)=>{if (e.key === 'Enter') e.preventDefault()}} placeholder="input notes here..." className="notes" />
        </div>
    )
}