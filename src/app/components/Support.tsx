...
export const Support = (props: { user: any }) => {
  ...
  const Form = (
    <div className="action-container-subcontainer">
      <Select
        ...
        placeholder="Subject"
        options={['General', 'Bug', 'Feature Request'].map((opt: string) => ({ label: opt, value: opt }))}
      />
      <div style={{ marginLeft: '5px', width: '97%', marginTop: '10px' }}>
        <input
          onInput={(e: any) => setEmail(e.currentTarget.value)}
          placeholder="Enter email address"
          className={`text-input${errors.includes('email') ? ' warning-border' : ''}`}
          type="email"
        />
      </div>
      <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
        <textarea
          ...
          maxLength={256}
          className={`support-text${errors.includes('message') ? ' warning-border' : ''}`}
          placeholder="Enter your message..."
        />
      </div>
      <div style={{ marginLeft: '10px' }}>
        <Button onClick={handleSubmit} secondary style={{ fontSize: '10px', height: '20px', lineHeight: '10px' }}>
          submit
        </Button>
      </div>
    </div>
  ...
  const Complete = (
    <div style={{ margin: '10px' }} className="action-container-subcontainer">
      <div>
        <h2>Message Sent</h2>
      </div>
      <div>
  ...
  return (
    <div id="action-container">
      <div className="action-container-content">
        <div>
          <p>
            <strong>Contact us.</strong> We want to hear from you.
          </p>
        </div>
      </div>
