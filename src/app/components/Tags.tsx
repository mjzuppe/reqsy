...
export const Tags = (props) => {
  ...
  const tagsFiltered = search
    ? tagsKeys.filter((key: string) => tagsObject[key].label.toLowerCase().includes(search.toLowerCase()))
    : tagsKeys;
  ...
  const invalidTags = (current: string) =>
    Object.values(tagsObject)
          ...
          >
            ...
            >
              <div style={{ marginLeft: '10px' }}>
                <TextInput
                  invalidlist={invalidTags('')}
                  id="tags-items-new-tag"
        ...
        {tagsFiltered.map((tag) => (
          ...
          >
            ...
            >
              <TextInput
                dblclick
                invalidlist={invalidTags(tagsObject[tag].label)}
                placeholder="enter tag..."
                style={{ marginLeft: '10px' }}
                maxLength={16}
                onblur={clickHandlerNewOrUpdate}
                defaultValue={tagsObject[tag].label}
                id={tag}
              />
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <Menu
                  danger
                  marginLeft={'-28px'}
                  onClick={() => clickHandlerDelete(tag)}
                  options={['delete?']}
                  trigger={
                    <IconButton>
                      <IconMinus32 />
