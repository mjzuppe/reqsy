...
export const Variables = (props) => {
    ...
    const variablesFiltered = search ? variableKeys.filter((v: any) => variableObject[v].label.toLowerCase().includes(search.toLowerCase())) : variableKeys;
    ...
    const invalidVariables = (current: string) => Object.values(variableObject).map((tagData: any) => tagData.label).filter((tagLabel: string) => tagLabel !== current);
    ...
    const clickHandlerNewOrUpdate = async (e: any) => {
        ...
        let variableId = newVariableOpen ? await generateReqsyId() : e.target.id.split("-")[e.target.id.split("-").length - 2];
        ...
        <>
            ...
            <div className="items-list">
                    ...
                    <div key={`variables-items-new-variable`} style={{ padding: "5px 0 5px" }} className="items-list-item items-border-bottom">
                        ...
                        <div className={`items-list-item-expanded`} style={{ padding: "5px 0 5px" }}  >
                            <div style={{ paddingLeft: "7px", alignItems: "center", width: "100%", display: "flex", justifyContent: "space-between" }}>
                                ...
                                <div>
                                    <TextInput placeholder="validation/notes" style={{ marginLeft: "2px", width: "160px" }} maxLength={32} onblur={clickHandlerNewOrUpdate} defaultValue={""}
                ...
                {variablesFiltered.map((variableId, i) => {
                    ...
                    return (
                        <div key={`variables-items-${variableId}`} style={{ padding: "5px 0 5px" }} className="items-list-item items-border-bottom">
                            <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center" }} className="items-list-item">
                                <TextInput dblclick invalidlist={invalidVariables(variableObject[variableId].label)} placeholder="enter label..." style={{ marginLeft: "10px" }} maxLength={16} onblur={clickHandlerNewOrUpdate} defaultValue={variableData.label} id={`variable-items-${variableId}-label`} />
                                <div style={{ display: "flex", justifyContent: "flex-end" }}>
