...
const parseTags = (text: string, validOptions: any[]) => {
  ...
  const chunks = text
    ...
    .map((chunk: string, i: number) =>
      chunk[0] === '@' && isValid(chunk) ? (
        <DetailCard text={chunk} type={chunkData(chunk).type || ''} description={chunkData(chunk).description || ''} />
      ) : (
        <span>{chunk}</span>
      )
    );
...
const InputMention = (props: {
  ...
  const { options, onBlur, onInput, defaultValue, placeholder, disabled, style, ...rest } = props;
  ...
  const handleKeyDown = async (e: any) => {
    ...
    if (openDropdown && (e.keyCode === 38 || e.keyCode === 40)) {
      ...
      if (
        newDropdownSelected > 0 &&
        newDropdownSelected <= filterOptions(optionsLabels, currentDropdownChunk.split('@')[1]).length
      )
        setDropdownSelected(newDropdownSelected);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'grid', placeItems: 'flex-start', height: '100%', gridTemplateAreas: 'inner-div' }}>
        {displayPlaceholder ? (
          <div style={{ padding: '1px', gridArea: 'inner-div', color: 'rgba(255, 255, 255, 0.4)' }}>{placeholder}</div>
        ) : (
          ''
        )}
        <div
          ...
          onInput={handleInput}
          style={{ padding: '1px', gridArea: 'inner-div' }}
          className="input-mention"
          onBlur={handleBlur}
          key={key}
          contentEditable={!disabled}
        >
          {parseTags(value, options)}
        </div>
      </div>
      {openDropdown && (
            ...
            <div className="menu">
              {filterOptions(optionsLabels, currentDropdownChunk.split('@')[1]).map((option, i) => (
                <div
                  ...
                  onClick={() => {}}
                  key={i}
                  className={`menu-option ${i === dropdownSelected - 1 ? 'active' : ''}`}
                >
                  {option}
                </div>
              ))}
            </div>
            <div className="menu-listener" onClick={() => setOpenDropdown(false)}></div>
          </div>
        </div>
      )}
    </div>
