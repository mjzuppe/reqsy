...
const lsRetrieveSubRenewal = async (ls_email: string) => {
  const r = await fetch(
    `https://api.lemonsqueezy.com/v1/subscriptions?filter[user_email]=${ls_email.split('+').join('%2b')}`,
    {
      method: 'GET',
      headers: {
...
const route = async (req: { pathname: string; method: string; body: any; search: string }) => {
  ...
  switch (pathname) {
    ...
    case '/api/activate': {
      ...
      const renews_at: string =
        !subscription || subscription.data.length === 0
          ? null
          : subscription?.data[subscription.data.length - 1]?.attributes?.renews_at;
      const a = await pg.queryArray(
        `UPDATE users SET ls_license_key = '${license_key.key}', renews_at = '${renews_at}', email_ls = '${meta.customer_email}' WHERE id_figma = '${body.id_figma}';`
      );
      user = await getUser(body.id_figma);
    ...
    case '/api/support': {
      ...
      const pg = await connection();
      await pg.queryArray(
        `INSERT INTO support_ticket (email, category, text, user_id) VALUES ('${body.email}', '${body.category}', '${body.text}', '${body.user_id}');`
      );
      break;
    }
    default: {
